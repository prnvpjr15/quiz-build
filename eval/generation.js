...
const dataset = JSON.parse(
  fs.readFileSync(path.join(__dirname, 'datasets', 'generation-prompts.json'), 'utf8')
);

const prompts = flags.limit ? dataset.prompts.slice(0, Number(flags.limit)) : dataset.prompts;

async function main() {
  if (!process.env.GEMINI_API_KEY) {
    console.error('GEMINI_API_KEY is required — this eval makes real model calls.');
    process.exit(1);
  }

  console.log(`Generation eval — ${prompts.length} prompts against ${process.env.GEMINI_MODEL || 'gemini-3.7-flash'}\n`);
  metrics.reset();
  ...
  for (const spec of prompts) {
    ...
    try {
      ...
      console.log(`  ok    ${spec.id.padEnd(16)} ${quiz.questions.length}/${spec.questionCount} questions  ${Date.now() - startedAt}ms`);
    } catch (err) {
  ...
  table([
    ...
    ['questions produced', `${quality.producedQuestions}/${quality.requestedQuestions}`],
    ...
    ['position skew (0 even, 1 all one slot)', quality.answerPositionSkew === null ? 'n/a' : String(quality.answerPositionSkew)],
  ]);
  ...
  table([
    ...
    ['p50 / p95 latency', snapshot.latencyMs.quiz
