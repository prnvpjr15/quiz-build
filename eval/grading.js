...
const dataset = JSON.parse(
	fs.readFileSync(
		path.join(__dirname, "datasets", "grading-cases.json"),
		"utf8",
	),
);

const cases = flags.limit
	? dataset.cases.slice(0, Number(flags.limit))
	: dataset.cases;
...
function reportMode(label, results) {
	...
	table([
		[
			"accuracy",
			`${percent(stats.accuracy)}  (${stats.matrix.truePositive + stats.matrix.trueNegative}/${stats.total})`,
		],
...
function listFailures(results) {
	...
	for (const failure of failures) {
		const direction = failure.expected
		...
		if (failure.judgeReason)
			console.log(`    judge:     ${failure.judgeReason}`);
	}
}
...
async function main() {
	...
	if (useJudge && !process.env.GEMINI_API_KEY) {
		console.error(
			"GEMINI_API_KEY is required for the full pipeline. Use --no-judge to run the deterministic stages only.",
		);
		process.exit(1);
	}
	...
	const legacyStats = reportMode(
		"1. Legacy exact match (pre-pipeline behaviour)",
		legacy,
	);

	metrics.reset();
	const deterministic = await runMode(async () => null);
	const deterministicStats = reportMode(
		"2. Normalization + fuzzy match (no model calls)",
		deterministic,
	);
	...
	if (useJudge) {
		...
		fullStats = reportMode(
			"3. Full pipeline (normalization + fuzzy + model judge)",
			full,
		);
		...
		// to "incorrect".
		if (degraded) {
			const fallbacks = full.filter(
				(r) => r.matchType === "none" && !r.judgeReason,
			);
  ...
  !! DEGRADED RUN — the judge was unreachable for ${snapshot.judge.unavailable} case(s).
	...
	table([
		...
		[
			...
			`${((finalStats.accuracy - legacyStats.accuracy) * 100).toFixed(1)} points`,
		],
	]);
	...
	if (baseline) {
		...
		if (baseline.judgeEnabled !== useJudge) {
			console.log(
				`  note: baseline ran with judge ${baseline.judgeEnabled ? "enabled" : "disabled"}, this run with it ${useJudge ? "enabled" : "disabled"} —`,
			);
			console.log(
				"        differences below reflect the mode change, not a regression.\n",
			);
		}
		table([
		...
		for (const change of drift.cases) {
			console.log(
				`  ${change.improved ? "fixed   " : "REGRESS "} ${change.id}: ${change.was} -> ${change.now}`,
			);
		}
	}
	...
	console.log(
		"Promote a run to the regression baseline with: npm run eval:baseline\n",
	);
}
