...
if (!fs.existsSync(latest)) {
	console.error(
		`No run to promote: ${path.relative(process.cwd(), latest)} does not exist.`,
	);
...
if (run.degraded && !process.argv.includes("--force")) {
	...
	console.error(
		"The judge was unreachable for some cases, so its accuracy is a lower bound.",
	);
	console.error(
		"Re-run the eval once quota allows, or pass --force if you know what you are doing.",
	);
	process.exit(1);
}
...
console.log(
	`Promoted run from ${run.ranAt} (model: ${run.model}) to ${path.relative(process.cwd(), baseline)}`,
);
