...
async function gradeShortAnswer(question, userAnswer, judge) {
  ...
  metrics.increment(judgement.correct ? 'shortAnswersSemantic' : 'shortAnswersRejected');
...
function gradeObjective(question, userAnswer) {
  switch (question.type) {
    case 'multiple-choice':
      return { correct: Number(userAnswer) === question.correctAnswerIndex, matchType: MATCH.EXACT };
    case 'true-false':
      return { correct: Boolean(userAnswer) === question.correctAnswer, matchType: MATCH.EXACT };
    default:
      return { correct: false, matchType: MATCH.NONE };
  }
}
