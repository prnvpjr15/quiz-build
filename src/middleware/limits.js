...
function generateLimiter() {
	return limiter({
		...
		message:
			"Quiz generation limit reached for this hour. Please try again later.",
	});
}
...
function dailyBudget(req, res, next) {
	...
	if (used >= cap) {
		logger.warn("daily generation budget exhausted", {
			requestId: req.id,
			used,
			cap,
		});
		return res.status(429).json({
			error:
				"This service has reached its daily quiz-generation limit. Please try again tomorrow.",
		});
	}
